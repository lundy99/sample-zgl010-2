...
function getClientCredentialsTokenByTenant(tenant,credentials,zoneInfo, cb) {
  return xssec.requests.requestClientCredentialsToken(tenant, credentials.uaa || credentials,null,zoneInfo && zoneInfo.zoneId || null,(err, accessToken,tokenInfo) => {
    if (err) {
...
function loadClientCredentialsToken(app, credentials, serviceName, cb) {
  xssec.requests.requestClientCredentialsToken(null, credentials.uaa || credentials,null,null,(err,accessToken, tokenInfo) => {
    if (err) {
      tracer.error('Error getting client_credentials token for ' + credentials.label + ' error ' + err.message);
...
function getClientCredentialsToken(subdomain, credentials,zoneId) {
  return new Promise((resolve,reject) => {
    xssec.requests.requestClientCredentialsToken(subdomain,credentials,null,zoneId, (err, token, tokenJson) => {
      if (err) {
