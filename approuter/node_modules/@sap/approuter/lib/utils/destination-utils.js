...
function getHttpRequestOptions(destination, opts){
  if (destination.certificate){
    switch (destination.certificate.type) {
    case 'p12':
      opts.pfx =  destination.certificate.buffer;
      opts.passphrase = destination.certificate.passphrase;
      // if certificate provided by XSUUA or IAS should be deleted
      opts.cert &&  delete opts.cert;
      opts.key  &&  delete opts.key;
      break;
    case 'pem':
      opts.cert =  destination.certificate.cert;
      opts.key = destination.certificate.key;
      break;
    }
  }
  return opts;
...
function getDestinationCertificate(destinationLookUpResult){
  ...
  const { KeyStoreLocation, KeyStorePassword } = destinationLookUpResult.destinationConfiguration ;
  ...
  certificates.forEach((certificate) => {
    ...
    if (Name === KeyStoreLocation){
      ...
      clientCertificates.buffer = Buffer.from(Content,'base64');
      clientCertificates.passphrase = KeyStorePassword;
      clientCertificates.type = extension;
    }
  });
  return Object.keys(clientCertificates).length > 0 ? clientCertificates : null ;
}
